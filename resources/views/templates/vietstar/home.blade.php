			...
			<div class="col-md-9">
				...
				<div class="row g-0">
					...
					<div class="card-body card-body-profile-seeker">
						...
						<p class="card-text justify-content-between align-items-center">{{ auth()->user()->getProfileSummary('summary') }}</p>
						{{--<a href="{{ route('view.public.profile', Auth::user()->id) }}" class="btn btn-primary btn-edit-profile">
							...
							<div class="card-body">
								...
								<div class="row related-jobs__jobs px-12px mb-2">
									...
									<div class="card-news__content">
										...
										<p class="card-news__content-detail">{{!empty($match->getCompany()) ? $match->getCompany()->name : ''}}</p>
										...
										<div class="card-news__content-footer__salary">
											{{$match->salary_from.' '.$match->salary_currency}} - {{$match->salary_to.' '.$match->salary_currency}}
										</div>
										</div>
									</div>
									</div>
				...
				<div class="col-md-5">
					...
					<div class="card-body">
						...
						@foreach($followers as $follow) 
							@php 
								$company = DB::table('companies')->where('slug',$follow->company_slug)->where('is_active',1)->first(); 
							@endphp
							@if (!empty($company))
