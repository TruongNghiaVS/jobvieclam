    ...
    <div class="tab-content">
        ...
        <section class="job-detail-title">
            ...
            <div class="job-detail-banner__group d-flex flex-fill">
                ...
                <div class="btn-group-vertical job-detail-banner__actions">
                @if(Auth::guard('company')->check())
                    @if(Auth::guard('company')->user()->id == $job->company_id )
                        <a href="{{route('edit.front.job', $job->id)}}" class="btn btn-outline-primary"> {{__('Edit Job')}} </a>
                    @endif
                @else
                    @if($job->isJobExpired())
                    ...
                    <button class="btn btn-primary apply applied" disabled><i class="fa fa-paper-plane iconawesome" aria-hidden="true"></i> {{__('Already Applied')}}</button>
                    @else
                    <a href="{{route('apply.job', $job->slug)}}" class="btn btn-primary apply"><i class="fa fa-paper-plane iconawesome" aria-hidden="true"></i> {{__('Apply Now')}}</a>
                    @endif
                    @if(Auth::check() && Auth::user()->isFavouriteJob($job->slug)) 
                    <a href="{{route('remove.from.favourite', $job->slug)}}" class="btn btn-outline-primary"><i class="fas fa-heart iconoutline"></i> {{__('Favourite Job')}} </a> @else <a href="{{route('add.to.favourite', $job->slug)}}" class="btn btn-outline-primary"><i class="fas fa-heart iconoutline"></i> {{__('Add to Favourite')}}</a> 
                    @endif
                @endif
                </div>
            </div>
            ...
            <div class="tab-content">
                ...
                <section class="job-detail-content">
                    ...
                    <div class="share-detail">
                        ...
                        <div class="__actions">
                            @if(Auth::check() && Auth::user()->isFavouriteJob($job->slug)) 
                                <a href="{{route('remove.from.favourite', $job->slug)}}" class="btn btn-outline-primary"><i class="fas fa-heart iconoutline"></i> {{__("Don't Save")}} </a> 
                            @else 
                                <a href="{{route('add.to.favourite', $job->slug)}}" class="btn btn-outline-primary"><i class="fas fa-heart iconoutline"></i> {{__('Save')}}</a> 
                            @endif
                            @if($job->isJobExpired())
                            <span  class="btn btn-primary jbexpire mb-2"><i class="fa fa-paper-plane iconawesome" aria-hidden="true"></i> {{__('Job is expired')}}</span>
                            @elseif(Auth::check() && Auth::user()->isAppliedOnJob($job->id))
            ...
            <!-- realated jobs -->
                <section class="related-jobs">
                    <div class="related-jobs__title d-flex justify-content-between align-items-center">
                        ...
                        <a class="btn btn-round btn-link btn-sm main-color" href="{{ route('job.list', ['job_titles[]'=>$job->title ?? '', 'country_ids[]'=>$job->country_id?? '', 'state_ids[]'=>$job->state_id?? '', 'city_ids[]'=>$job->city_id ?? '']) }}"> {{ __('View all jobs') }}<i
                                    ...
                                    <div class="card-news__content">
                                        ...
                                        <div class="card-news__content-footer">
                                            ...
                                            <div class="card-news__content-footer__salary">
                                                {{$relatedJob->salary_from.' '.$relatedJob->salary_currency}} - {{$relatedJob->salary_to.' '.$relatedJob->salary_currency}}
                                            </div>
                                        </div>
                                    </div>
                                </div>
        ...
        <section class="company-job-detail job-detail-title">
            ...
            <div class="job-detail-banner__detail">
                ...
                <div class="col-xl-7 col-lg-12">
                    ...
                    <div class="banner__company">{{ !empty($company->industry)?$company->industry->industry : 'NA' }}</div>
                    <div class="banner__due-day mb-3">
        ...
        <div class="row">
            ...
            <!-- company detail -->
            <section class="job-detail-content mb-4">
                <div class="require-card">
                <div class="row">
                        ...
                        @foreach ($company->jobs as $cjob)
                            <div class="card-news gap-16 mb-2">
                                ...
                                <div class="card-news__content">
                                    ...
                                    <div class="card-news__content-footer__salary">
                                        {{$cjob->salary_from.' '.$cjob->salary_currency}} - {{$cjob->salary_to.' '.$cjob->salary_currency}}
                                    </div>
                                    </div>
                                </div>
                            </div>
