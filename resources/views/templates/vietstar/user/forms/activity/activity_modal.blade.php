    ...
    <div class="modal-content">
        <form class="form" id="add_edit_profile_activity" method="POST" action="{{ route('store.front.profile.activity', [$user->id]) }}">{{ csrf_field() }}
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-large btn-primary" onClick="submitProfileActivityForm();">{{__('Add Activity')}} <i class="fa fa-arrow-circle-right" aria-hidden="true"></i></button>
            </div>
        </form>
    </div>
    <!-- /.modal-content --> 
